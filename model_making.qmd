...
#data <- read.csv(final_lead_limited_data_weather.csv")
...
#RMSE Visualizations
```{r}
#either fill this out with all models or delete
#just nice to show him we can do visuals too lol
...
test_pred <- predict(extract_workflow(final_fit), data_test)

#Gathering RMSE
test_rmse <- data_test |>
  mutate(.pred = test_pred$.pred) |>
...
```
Out of Sample RMSE = 7.9286
Not too shabby

#Final Predictions
```{r}
predictions <- predict(extract_workflow(final_fit), bike_implementation)
